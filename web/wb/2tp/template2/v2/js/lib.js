...
function arrReplace(array,obj,obj2){
	var result=[];
	for(var i=0;i<array.length;i++){
		if(array[i]==obj){
...
function cssHave(dom,css1){/*判断是否包含此样式*/
	if(isStr(dom))	dom=$(dom);
...
function cssToggle(dom,css1,css2){/*开关样式*/
	if(isStr(dom))	dom=$(dom);

	if(isNodes(dom)){
		arrEach(dom,function(e){
				cssToggle(e,css1,css2);
			});
	}
	else if(isDom(dom)){
		var css=dom.className.split(/\s+/g);
...
function cssReplace(dom,css1,css2){/*样式替换*/
	if(isStr(dom))	dom=$(dom);

	if(isNodes(dom)){
		arrEach(dom,function(e){
				cssReplace(e,css1,css2);
			});
	}
	else if(isDom(dom)&&dom.className){
...
function cssActive(dom,cfg){/*激活某一项dom，将同级元素都隐藏  active  normal*/
	/*active样式  normal样式*/
	if(isStr(dom))	dom=$(dom);
	var doms=dom.parentNode.childNodes;
	arrEach(doms,function(e){
			if(e&&e.style&&e!=dom)	e.style.display="none";
		})
	dom.style.display="";
}

function bindClick(dom,fun,obj){
	bind(dom,"click",fun,obj);
}

function bind(dom,evt,fun,obj){/*obj是要绑定的数据对象*/
	if(isStr(dom))	dom=$(dom);
	if(isNodes(dom)) dom.forEach(function(e){bind(e,evt,fun,obj)});
	else if(isDom(dom)){
		if(obj==null) obj=dom;
		if(isIE>0&&isIE<9){
			var fun2=function(){
...
function json(file,callback){
	return xh("/file/"+file+".json",null,false,callback);
}
function ajax(path,param,post,callback){
	return xh("/handler/"+path,param,post,callback);
}
...
function iframeAutoHeight(dom){
	var frame=getSelfIframe();
	if(frame==null)	return;
	//var node=window.document.getElementById("con");
	//console.log("node.height="+dom.scrollHeight);
	frame.style.height=dom.scrollHeight+"px";
