...
// CommonJS require()
var FS={readFileSync:function(path,cfg){
		var dir="";
...
function getFileContent(path,dir){
	if(dir)		path=dir+path;
	//console.log("get Path:"+path);
	path=path+".ejs";
	
	var xh=new XMLHttpRequest(path);
	...
	/*动态把include的内容都替换掉*/
	var matches=text.match(/<include\s+[a-z|A-Z|0-9|]+>/g);
	if(matches){
		matches.forEach(function(e,i){
			var file=e.replace(/\<include\s+([a-z|A-Z|0-9|]+)>/,"$1");
			var content=getFileContent(file,dir);
			
			text=text.replace(e,content);
		});
	}
